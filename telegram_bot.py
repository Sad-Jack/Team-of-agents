...
async def _run_dry(update: Any, user_text: str, session_id: str, user_id: str, channel: str) -> None:
    if not user_text:
...
async def _run_execute(update: Any, user_text: str, confirmed: bool, session_id: str, user_id: str, channel: str) -> None:
    if not user_text:
...
async def handle_user_text(
    ...
    if not execute_mode:
        await _run_dry(update, user_text, session_id=session_id, user_id=user_id, channel=channel)
        return
    await _run_execute(
        update,
        user_text,
...
async def start_handler(update: Any, context: Any) -> None:
    ...
    if not is_owner(update, cfg["owner_id"]):
        ...
        return
    text = (
        "Привет. Это интерфейс Team of Agents через Telegram.\n"
        "Режим: owner-only.\n"
        "По умолчанию используется dry-run (настраивается).\n"
        "Примеры: /dryrun Создай задачу на healthcheck, /execute Покажи backlog"
    )
