...
### Уведомления, которые приходят в status chat

| Событие | Сообщение |
|---|---|
| create_task | `🆕 Создана задача: TASK-X\nНазвание: ...` |
| create_bug | `🐞 Создан баг: TASK-X\nНазвание: ...` |
| run_next / advance | `✅ Работа по TASK-X завершена\nНовый статус: ...` |
| Ошибка выполнения | `❌ Ошибка при выполнении действия\nAction: ...\nПричина: ...` |

Если `TELEGRAM_STATUS_CHAT_ID` не задан — уведомления не отправляются.
